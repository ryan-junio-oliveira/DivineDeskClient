<template>
    <div v-if="showRegisterModal" class="modal fade show" style="display: block;" tabindex="-1" role="dialog"
        aria-labelledby="registerOfferModalLabel" aria-hidden="true">
...
export default {
    ...
    methods: {
        async submitForm() {
            try {
                await offerService.addOffers(this.form);
                this.$emit('offer-registered');
                this.closeModal();
                this.showNotification('Oferta registrada com sucesso!', 'success');
            } catch (error) {
                this.closeModal();
